  ...
  <div class="dashboard-editor-container">
    ...
    <el-card class="box-card" style="height: 1200px;">
      <el-form  label-width="10px">
        <el-form-item label="">
          <el-input v-model="status" placeholder="请输入状态" style="width: 150px"></el-input>
          <el-input v-model="showData" placeholder="请输入展示数据" style="width: 150px"></el-input>
      ...
      <el-table :data="list" style="width: 100%;padding-top: 10px;margin-top: 10px" border="">
        ...
        <el-table-column prop="paramName" label="展示数据" align="center">
        </el-table-column>
        <el-table-column prop="paramType" label="内容状态" align="center">
        </el-table-column>
        <el-table-column prop="isSystem" label="是否系统服务器" align="center">
        </el-table-column>
        <el-table-column prop="isDefault" label="是否默认" align="center">
        </el-table-column>
        <el-table-column prop="sortOrder" label="排列顺序" align="center">
        </el-table-column>
    ...
    <el-dialog title="基础信息详情" :visible.sync="dialogFormVisible" size="small" label-width="150px" width="80%">
      <el-form ref="form" :model="form" label-width="100px" style="width: 50%;margin-left: 20%;margin-top: 20px">
        <el-form-item label="展示数据">
          <el-input v-model="form.showData" ></el-input>
        </el-form-item>
        <el-form-item label="内容状态">
          <el-input v-model="form.status" ></el-input>
        </el-form-item>
        <el-form-item label="是否系统默认">
          <el-select v-model="form.isSystem"  placeholder="请选择" >
            <el-option
        ...
        <el-form-item label="是否默认">
          <el-select v-model="form.isDefaut"  placeholder="请选择" >
            <el-option
...
export default {
  ...
  methods: {
    fetchData() {
      const _this = this
      this.$fetch('baseparam/list?paramName=' + this.status + '&paramCode=' + this.showData + '&start=' + (this.currentPage - 1) * 10 + '&length=10')
        .then((data) => {
          console.log(data)
    ...
    onSubmit() {
      ...
      this.$post('baseparam/save?id=' + this.form.id + '&paramName=' + this.form.showData + '&paramCode=' + this.form.status + '&isSystem=' + isSystem + '&isDefault=' + isDefaut + '&sortOrder=' + this.form.sort + '&_csrf=' + 'a73924c9-7aab-475a-8ae2-3e4c6d3568d6')
        .then((data) => {
          console.log(data)
          if (data.code === '20000') {
            _this.$notify({
